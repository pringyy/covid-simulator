...
const Map = (props) => {
  ...
  let today = new Date();
  let todayString = `${today.getFullYear()}-${today.getMonth()}-${today.getDate()-5}`
  ...
  useEffect(()=>{
    axios.get(endpoint).then((res)=> {setData(res.data.data);
      const maxVal = Math.max.apply(Math, res.data.data.map((x) =>{ return x.cumCasesBySpecimenDateRate; }));
      colorScale = scaleLinear().domain([0, maxVal])
                                .range(["#ffedea", "#ff5233"]);
      ...
      >
        <ZoomableGroup zoom={30} minZoom={30} maxZoom={100} center={[-2, 55]}>
          <Geographies geography={geoUrl}>
            {/*({ geographies }) =>
            geographies.map((geo) => {
              const d = data.find((s) => s.LAD13NM === geo.properties.LAD13NM);
              return (
                <Geography
                  key={geo.rsmKey}
                  geography={geo}
            ...
            {({ geographies }) =>
              ...
              const d = data.find((s) => s.area === geo.properties.LAD13NM)
                ...
                <Geography
                  ...
                  fill={d ? colorScale(d["cumCasesBySpecimenDateRate"]) : "#F5F4F6"}
